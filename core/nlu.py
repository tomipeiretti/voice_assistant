...
_re_web = re.compile(r"\bbusca?r?\s+en\s+internet\b|\bbusca?r?\b", re.I)
_re_play = re.compile(r"\b(reproducir|reproduce)\b", re.I)
_re_joke = re.compile(r"\bchiste\b", re.I)
_re_cpu = re.compile(r"\buso\s+de\s+cpu\b", re.I)
_re_mem = re.compile(r"\buso\s+de\s+(memoria|ram)\b", re.I)
...
_re_calc = re.compile(r"\b(calcula?r?|cuánto\s+es)\b", re.I) #hacer calculos

_re_bye = re.compile(r"\badiós\b|\bchau\b|\bhasta luego\b|\bsalir\b|\bnos vemos\b|\bchao\b", re.I)
...
class SimpleNLU:
    def parse(self, text: str) -> Dict[str, Any]:
        ...
        if m:
            return {"intent": "mem_at_time", "entities": {"time": m.group(2)}}
        if _re_cpu.search(t):
